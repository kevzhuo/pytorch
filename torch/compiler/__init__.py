...
def export_python(
    *,
    ...
    .. warning::
        ...
        on a narrower ISA silently leaves most of each vectorized strip of the output
        unwritten -- roughly half the tensor is uninitialized memory for an AVX-512
        artifact loaded under AVX2 -- with no error at all. Running a CUDA artifact on a
    ...
    ``out=None`` parameter described below. Four things that ``make_fx`` specializes
    without emitting a guard are recorded as comment stamps and checked on every call:
    ...
    kernels were built for). What is *not* guarded is a change in *how* two aliased
    ...
    ``torch.compile`` has the same hole. A hand-edit that drops a stamp turns that one
    check off with a warning, and ``unsafe_reduce_overhead`` skips all four whenever it
    takes effect. All five stamps (these four plus the version stamp) must stay in the
