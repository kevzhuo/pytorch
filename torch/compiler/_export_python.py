...
def _autocast_state(
    ...
    # Only device types the inputs actually live on: a CPU helper first called inside a
    # `with torch.autocast("cuda")` training region must not be locked to that region.
    if tensors is None:
        tensors = _input_tensors(args)
...
class ExportedPythonArtifact:
    ...
    def _precompile_and_save(self, args: tuple[Any, ...]) -> tuple[str, bool]:
        ...
        code = (
            ...
            f"{_AUTOCAST_TAG}{_autocast_state(example, example_tensors)!r}\n{code}"
        )
    ...
    def _check_capture_environment(self, args: tuple[Any, ...]) -> None:
        ...
        if self._autocast is None:
            ...
            actual_autocast = _autocast_state(args, input_tensors())
